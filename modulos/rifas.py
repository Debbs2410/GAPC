...
def gestionar_rifas(id_distrito=None, id_grupo=None):
    ...
    with st.form("form_rifa"):
        ...
        ciclo_sel = st.selectbox("Ciclo", ciclo_opciones, index=ciclo_default)
        id_ciclo = ciclos_dict[ciclo_sel]
        nombre_rifa = st.text_input("Nombre de la rifa")
        fecha = st.date_input("Fecha de la rifa")
        monto = st.number_input("Monto recaudado ($)", min_value=0.0, step=0.01, format="%.2f")
        descripcion = st.text_area("Descripción de la actividad")
        submitted = st.form_submit_button("Registrar Rifa", type="primary")
        if submitted:
